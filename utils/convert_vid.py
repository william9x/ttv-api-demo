...
def create_video_filter(orig_file_name):
    ...
    result = subprocess.run(['ffprobe', '-show_streams', '-select_streams', 'v', '-of', 'json', orig_file_name],
...
def convert_file(orig_file_name):
    ...
    command = 'ffmpeg -i "%s" %s  -acodec aac -strict -2 -ab %sk -ar 44100 -vcodec h264 -vb %sK -y -movflags +faststart "%s" ' % (
